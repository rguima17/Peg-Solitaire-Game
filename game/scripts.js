  ...
  for (let i = 0; i < board.length; i++) {
    ...
    for (let j = 0; j < board[i].length; j++) {
      ...
      if (piecePosition === null) {
        ...
        piecePositionDiv.appendChild(
          createPiece("emptyPiece zeroPosition", i, j)
        );
      }
  ...
  for (let i = 0; i < boardPositions.length; i++) {
    boardPositions[i].onclick = function () {
      ...
      if (classOfSelectedPosition.indexOf("occupiedPosition") > -1) {
        ...
        classOfSelectedPosition.indexOf("emptyPiece") > -1 &&
...
function jumpIsValid(startPosition, finalPosition) {
  if (allRows.length === 5) {
    return !(
      finalPosition.className.indexOf("emptyPiece") < 0 || //Check if Empty
      ...
      !(
        ...
        (Math.abs(startPosition["data-row"] - finalPosition["data-row"]) ===
          2 &&
          Math.abs(
            startPosition["data-column"] - finalPosition["data-column"]
          ) === 2)
      ...
      getPieceInMiddle(startPosition, finalPosition).className.indexOf(
        "occupiedPosition"
      ) < 0
    );
  } else {
...
function checkIfCanWin() {
  for (let i = 0; i < boardPositions.length; i++) {
    for (let j = 0; j < boardPositions.length; j++) {
      if (
        boardPositions[i].className.indexOf("occupiedPosition") > -1 &&
        jumpIsValid(boardPositions[i], boardPositions[j])
      ) {
        return true;
      }
    }
...
function getPieceInMiddle(startPosition, finalPosition) {
  ...
  if (startPosition["data-row"] === finalPosition["data-row"]) {
    ...
    colMiddle =
      startPosition["data-column"] > finalPosition["data-column"]
        ? startPosition["data-column"] - 1
        : finalPosition["data-column"] - 1;
  } else if (
    Math.abs(startPosition["data-row"] - finalPosition["data-row"]) === 2 &&
    ...
    rowMiddle =
      startPosition["data-row"] > finalPosition["data-row"]
        ? startPosition["data-row"] - 1
        : finalPosition["data-row"] - 1;
    colMiddle =
      startPosition["data-column"] > finalPosition["data-column"]
        ? startPosition["data-column"] - 1
        : finalPosition["data-column"] - 1;
  ...
  for (let i = 0; i < boardPositions.length; i++) {
    ...
    if (
      pieceInMiddle["data-row"] === rowMiddle &&
...
function resetGame() {
  for (let i = 0; i < boardPositions.length; i++) {
    if (
      boardPositions[i].className.indexOf("emptyPiece") > -1 &&
      ...
      boardPositions[i].className.indexOf("youLostPosition") > -1 &&
      boardPositions[i].className.indexOf("zeroPosition") < 0
    ) {
      boardPositions[i].classList.remove("youLostPosition");
      boardPositions[i].classList.add("occupiedPosition");
